 ...
 * Optional: X_EMAIL_OR_PHONE can be set for X's verification challenge.
 */
...
function getVerificationValue(username: string) {
  return process.env.X_EMAIL_OR_PHONE || username;
}
...
async function main() {
  ...
  const xVerificationValue = getVerificationValue(xUsername);
  ...
  if (isVerifyVisible) {
    console.log("[login-x] Verification prompt detected — entering email/phone fallback...");
    await verifyInput.click();
    await verifyInput.fill("");
    await page.waitForTimeout(200);
    await humanType(page, 'input[data-testid="ocfEnterTextTextInput"]', xVerificationValue);
    await page.waitForTimeout(400);
